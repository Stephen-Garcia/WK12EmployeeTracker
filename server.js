  ...
  const { roleId } = await prompt([
    {
      ...
      choices: (await DB.findAllRoles())[0].map(({ id, title }) => ({
        name: title,
        value: id,
      })),
    },
  ...
  const { managerId } = await prompt([
    {
      ...
      choices: (await DB.findAllPossibleManagers(employeeId))[0].map(({ id, first_name, last_name }) => ({
        name: `${first_name} ${last_name}`,
        value: id,
      })),
    },
...
async function removeDepartment() {
  ...
  async function addEmployee() {
    ...
    const employee = { manager_id: managerId, role_id: roleId, first_name, last_name };
